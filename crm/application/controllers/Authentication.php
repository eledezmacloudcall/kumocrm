...
class Authentication extends CI_Controller
{
    ...
    public function panel()
    {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                $success = $this->Authentication_model->login_panel($this->input->post('email'), $this->input->post('password'));
                if (is_array($success) && isset($success['memberinactive'])) {
                    set_alert('danger', _l('admin_auth_inactive_account'));
                    redirect(site_url('authentication/panel'));
                } elseif ($success == false) {
                    set_alert('danger', _l('admin_auth_invalid_email_or_password'));
                    redirect(site_url('authentication/panel'));
                }
        ...
        $data['title'] = _l('admin_auth_login_heading');
        $this->load->view('authentication/login_panel', $data);
    }
