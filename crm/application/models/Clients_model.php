...
class Clients_model extends CRM_Model
{
    ...
    {
        $idcompany = get_staff_company_id();
        ...
        if (is_numeric($id)) {
            ...
            $this->db->where('tblclients.companyid', get_staff_company_id());
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false)
    {
        ...
        $data['companyid']  = get_staff_company_id();
        $ps_not_hashed  = '';
    ...
    public function add($data, $client_or_lead_convert_request = false)
    {
       ...
       $data_company_client = array();
        ...
        /* Campos para la tabla km_companies */

        $admin_staffid = trim($data['admin_staffid']);
        $active = 1;
        $company = trim($data['company']);
        $address = trim($data['address']);
        $address2 = trim($data['address']);
        $city = trim($data['city']);
        $state = trim($data['state']);
        $country = trim($data['country']);
        $zip = trim($data['zip']);
        $phonenumber = trim($data['phonenumber']);
        $mobilephone = trim($data['phonenumber']);
        $taxnumber = '41521';

       $data_company_client = array('admin_staffid'=>$admin_staffid, 'status' => '1', 'companyname'=>$company, 'address1'=>$address, 'address2'=> $address, 'city'=>$city, 'state'=>$state, 'country'=>$country, 'zipcode'=>$zip, 'phone'=>$phonenumber, 'mobilephone'=>$mobilephone, 'taxnumber'=>$taxnumber);


        unset($data['admin_staffid']);
        ...
        /* Inserta los datos en la tabla km_companies */

       // $this->db->insert('km_companies', $data_company_client); 
       // $kmcompanyid = $this->db->insert_id();
        $kmcompanyid = get_staff_company_id();

        $data['companyid'] =$kmcompanyid;

        /* Inserta los datos del cliente (compañia para cloudcall) */
    ...
    public function get_groups($id = '')
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);
            $this->db->where('companyid', get_staff_company_id());
